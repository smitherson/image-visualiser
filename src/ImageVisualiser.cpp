...
void ImageVisualiser::redrawImage() {
    ...
    if (currentRect.x + originalSize.width > zoomedWidth)
        currentRect.x = currentRect.x - (originalSize.width - (zoomedWidth - currentRect.x));
    if (currentRect.y + originalSize.height > zoomedHeight)
        currentRect.y = currentRect.y - (originalSize.height - (zoomedHeight - currentRect.y));
